...
export const getCampDetails = createServerFn({ method: "POST" })
...
export const getShepherdMembers = createServerFn({ method: "POST" })
...
export const getCampDashboard = createServerFn({ method: "POST" })
  .inputValidator((data: { campId: string }) => data)
  .handler(async ({ data }) => {
    try {
      // Get camp info
